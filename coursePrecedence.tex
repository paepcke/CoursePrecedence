...
\begin{abstract}
We present a procedure that transforms ordinary post-secondary
enrollment data into formal graphs. Underlying our work are eighteen
years of one research university's enrollment records. Once the graph
is constructed, we demonstrate how interactive manipulation of the
graph helps answer questions of concern to as varied an audience as
students, instructors, and administrators. The combination of the data
transform procedure with subsequent computational and visual discovery
methods comprises our toolkit {\em Via}. At several stages of the
toolkit's use its users may add customizations that help answer
particular research or information gathering questions. We show that
the presented approach recovers independently obtained observations
from the literature.
 \end{abstract}
...
The dangers of choice overload are well documented in areas other than
education. Marketing \cite{botti2006} and psychology
\cite{iyengar2000} have observed both the stated preference for
choice, and improved factual success when choice is limited.

For the education context, \cite{bake2018} find that simply reducing
choice is not an acceptable answer. While students do not have strong
reactions to increased guidance, they do react negatively to a
reduction in choice. Thus enriching guidance is one promising
...
Community colleges are at least as affected by the tension between
ample choice and confusion as four year colleges. Educational
outcomes in community colleges are negatively affected by ``chaotic
enrollment patterns'' \cite{crosta2014, bail2015, scot2015}.

A number of post secondary institutions have explored options for
improved support by providing ``guided pathways'' \cite{jenkins2013}.
...
coloring of the nodes to indicate a node's relative betweenness
...
properties. Instead, we seek to model the probability of two courses
being taken sequentially given data in which the classes may have been
...
modeling students' course taking behavior at universities. Courses can
...
relationships between courses was obtained directly from a large
American private research university. This dataset contains the
anonymized enrollment data of over 52,000 students (both graduates and
undergraduates) who were enrolled at the university during any time
...
American university. {\em Via} allows stakeholders as diverse as
...
construction, and from the interactive manipulation of the
...
be shaped and reshaped to discover patterns. As post secondary
...
required. We have presented one step towards such capabilities.
